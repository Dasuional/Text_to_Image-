...
def Encoder_output(input_text):
    ...
    max_len = 100
    padded_tensor = torch.zeros(max_len, dtype=torch.int32)
    padded_tensor[:len(numericalized)] = torch.tensor(numericalized)[:max_len]

    padded_tensor = padded_tensor.unsqueeze(0)
    enc_output = transformer(padded_tensor)
